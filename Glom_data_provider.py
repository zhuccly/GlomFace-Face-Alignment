...
def cofw_reference_shape(paths, diagonal=200):
    """Builds the reference shape.
    Args:
      paths: paths that contain the ground truth landmark files.
      diagonal: the diagonal of the reference shape in pixels.
...
def load_cofw68(path, reference_shape, is_training=False, group='PTS',
    ...
    # im.landmarks['bb'] = mio.import_landmark_file(str(Path('bbs') / bb_root / (im.path.stem.replace(' ', '') + '.pts')))
    # im.landmarks['bb'] = im.landmarks[group].lms.copy().bounding_box()
    # im = im.crop_to_landmarks_proportion(0.3, group='bb')
    im, trans = crop_image_bounding_box(im, im.landmarks['bb'], [256, 256], base=1, order=1)
...
def batch_inputs(paths,
    ...
    # offset_h = tf.to_int32((tf.to_int32(256) - 198) / 2)
    # offset_w = tf.to_int32((tf.to_int32(256) - 198) / 2)
    # image = tf.image.crop_to_bounding_box(
    #     image, offset_h, offset_w, 198, 198)
    # lms = lms - tf.to_float(tf.stack([offset_h, offset_w]))
...
def cofw68_inputs(paths,
    ...
    print (filename)
    image, lms, lms_init, bbs = tf.py_func(
        partial(load_cofw68, is_training=is_training,
                mirror_image=mirror_image),
        [filename, reference_shape], # input arguments
        [tf.float32, tf.float32, tf.float32, tf.float32], # output types
        name='load_image'
